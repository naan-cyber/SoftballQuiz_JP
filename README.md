...
まもる人や走る人が、ボールの動きに合わせて「はじめに何をするか」をクイズ形式で練習するアプリです。画面で守備のポジション、バッターランナー、1るいランナー、2るいランナー、3るいランナー、きほんルール（アウト・フォース・タッチ、フェア・ファウル、ストライク・ボール、試合の流れ、野球とのちがい）の項目を選べます。
...
Flet のウィンドウは起動しませんが、クイズロジック・問題データに加えて画面補助ロジック（図解・ふりがな変換）も検証するため、Flet を含む依存関係が必要です。
